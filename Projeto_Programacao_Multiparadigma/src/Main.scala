...
class Main extends Application {
  ...
  override def start(stage: Stage): Unit = {
    ...
    val octree: oct = TextInterface.mainloop(oct(OcEmpty))
    val particoes = octree.getCubos
    val shapes = octree.getShapes
    ...
    Functions.putShapes(shapes, worldRoot)
    Functions.putParticoes(particoes, worldRoot)
    Functions.updateParticoes(worldRoot, camVolume,0, worldRoot.getChildren.size())
    ...
    scene.setOnKeyPressed( keyEvent => {
      removeParticoes(particoes)
      removeShapes(shapes)
      octree = octree.scale(2.0)
      particoes = octree.getCubos()
      shapes = octree.getShapes()
      putParticoes(particoes)
      putShapes(shapes)
      IO_Utils.update(worldRoot, camVolume,particoes)
    })*/

    /* Testar T5-change colour
    scene.setOnKeyPressed( keyEvent => {
      removeParticoes(particoes)
      removeShapes(shapes)
      octree = octree.mapColourEffect(greenRemove)
      particoes = octree.getCubos()
      shapes = octree.getShapes()
      putParticoes(particoes)
      putShapes(shapes)
