...
void init_ipc() {
    ...
    if(msg_id < 0){
        erro("C1) Fila de Mensagens com a Key definida não existe ou não pode ser criada");
        exit(1);  
    }
...
void cria_mensagem() {
    ...
    sucesso("C2.1) Dados do cidadão: %d, %s", mensagem.dados.num_utente, mensagem.dados.nome);
...
void envia_mensagem_servidor() {
    ...
    if(status < 0){
        erro("Não é possivel enviar mensagem ao servidor");
        exit(1);
    }
    sucesso("Mensagem para o servidor enviada");
...
void espera_resposta_servidor() {
    ...
    if(status < 0){
        erro("Não é possivel ler a resposta do servidor");
        exit(1);
    }
    sucesso("Servidor enviou resposta");
...
void pedido() {
    ...
    switch(resposta.dados.status){
        ...
        erro("C5.1) Não exite enfermeiro na localidade do utente %d, %s", mensagem.dados.num_utente, mensagem.dados.nome);
        exit(1);
        break;
...
void cancela_pedido(int sinal) {
    ...
    if(resposta.dados.status == CANCELADA){
        ...
        sucesso("C7.4.2) A vacinação já tinha sido concluida");
        exit(1);
    }
